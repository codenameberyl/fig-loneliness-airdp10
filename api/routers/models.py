...
async def get_best_per_representation():
    """Return the best model for each text representation."""
    return _require("best_per_representation.json")


@router.get("/test_report")
async def get_test_report():
    """Return the full classification report for the best model on the test set."""
    return sanitise_for_json(_require("test_evaluation_report.json"))


@router.get("/features_metadata")
async def get_features_metadata():
    """Return metadata about each feature representation (n_features, availability)."""
...
async def get_confusion_matrix(model_key: str):
    """
    Return the confusion matrix JSON for a specific model.

    model_key format: {representation}_{model}  e.g. tfidf_logistic_regression
    """
...
async def get_roc_curve(model_key: str):
    """
    Return the ROC curve data (fpr, tpr, thresholds, AUC, optimal threshold)
...
async def get_full_report(model_key: str):
    """
    Return the combined evaluation report (classification report + confusion matrix
    + ROC curve data) for a specific model.
    """
...
async def get_interpretability_summary():
    ...
    Includes interpretability scores, rationale, computational cost,
    and top discriminative features per representation.
    """
...
async def get_lr_coefficients(representation: str):
    """
    Return top-50 LR coefficients for a representation.
    Available for: linguistic_only, tfidf, tfidf_ling.
    """
...
async def get_distilbert_attention():
    """
    Return DistilBERT average last-layer attention weights per class.
    """
...
async def list_confusion_matrix_keys():
    """List all model keys that have a saved confusion matrix JSON."""
...
async def list_roc_curve_keys():
    """List all model keys that have a saved ROC curve JSON."""
    json_dir = RESULTS_SUBDIRS["json"]
...
async def list_eval_plots():
    ...
    eval_plots = [p for p in all_plots if p.startswith("eval_")]
